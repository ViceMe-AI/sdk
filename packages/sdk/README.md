...
`Error` reason for requests, interactive presentations, and cached session
operations such as `auth.getState()` and `access.refresh()`. Cancellation also
prevents late session authentication or response writeback; a request timeout
